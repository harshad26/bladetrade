...
class PersonMailer < ActionMailer::Base
  ...
  def send_email_of_remove_user_account(email_address, community)
    premailer_mail(:to => email_address,
         :from => community_specific_sender(@community),
         :subject => t("emails.welcome_email.user_destroy_message")) do |format|
       format.html { render :layout => false }
     end
  end
  # Added By Harshad Patel
  def send_email_of_remove_user_account_to_admin(to_email, email_address, community)
    @to_email = to_email
    subject = "User #{@to_email} has deleted their account"
    premailer_mail(:to => email_address,
         :from => community_specific_sender(@community),
  ...
  #--- Added By Harshad Patel
  def user_signup_email_admin(person, community, regular_email=nil, test_email=false)
    @recipient = person
    set_locale @recipient.locale
    @current_community = community
    @regular_email = regular_email
    @username = person.given_name_or_username
    @url_params = {}
    @url_params[:host] = "#{@current_community.full_domain}"
    @url_params[:locale] = @recipient.locale
    @url_params[:ref] = "welcome_email"
