...
class TransactionMailer < ActionMailer::Base
  ...
  def transaction_inform_to_seller(transaction)
    @transaction = transaction
    @community = @transaction.community

    subject = "Your item (#{@transaction.listing.title}) has been sold!"
    payment = @transaction.payment
    @person = payment.recipient
    @buyer = @transaction.starter
    
    @btn_url = @community.full_domain + new_person_person_message_path("en",@buyer.id)
    
    @click_here = "<a href='"+@btn_url+"'>Click here</a>"
    to_email = @person.emails[0].address
    premailer_mail(:to => to_email,
         :from => community_specific_sender(@community),
         :subject => subject) do |format|
      format.html { render :layout => false, locals: {
        name: @person.given_name,
        item_name: @transaction.listing.title,
        seller_username: @person.username,
        payer_full_name: payment.payer.name(@community),
        buyer_address: @buyer.location.address,
  ...
  # Added By Harshad Patel
  def transaction_inform_to_user(transaction)

    @transaction = transaction
    @community = @transaction.community

    subject = "You have purchased (#{@transaction.listing.title})"
    @person = @transaction.starter
    
    @btn_url = @community.full_domain + new_person_person_message_path("en",@person.id)
    
    @click_here = "<a href='"+@btn_url+"'>Click here</a>"
    to_email = @person.emails[0].address
    premailer_mail(:to => to_email,
         :from => community_specific_sender(@community),
         # :template_path => 'transaction_mailer/transaction_inform_to_user',
         :subject => subject) do |format|
      format.html { render :layout => false, locals: {
        name: @person.given_name,
        item_name: @transaction.listing.title,
