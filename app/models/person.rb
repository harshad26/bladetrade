...
#  validates_uniqueness_of :username
  ...
  def self.find_for_google_oauth(google_data, logged_in_user=nil)
    data = google_data.extra.raw_info
    ...
    if user = self.find_by_google_id(data.id)
      ...
      # make connection automatically based on email
      user.update_attribute(:google_id, data.id)
      if user.image.nil?
        self.picture_from_url(user.image)
      end
      user
    else
      nil
