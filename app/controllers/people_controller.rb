...
class PeopleController < Devise::RegistrationsController
  ...
  # after_filter :send_destroy_email, :only => [:destroy]
  ...
  def create_google_based
    ...
    # We trust that Facebook has already confirmed these and save the user few clicks
    Email.create(:address => session["devise.google_data"]["email"], :send_notifications => true, :person => @person, :confirmed_at => Time.now)
    ...
    # But as this is bit complicated, for now
    # we don't create the community membership yet, because we can use the already existing checks for invitations and email types.
    session[:google_join] = "pending_analytics"
    redirect_to :controller => :community_memberships, :action => :new
  end
  ...
  def destroy
    if @person && @current_user && @person == @current_user
      ...
      # Added By Harshad Patel
      admin_email = APP_CONFIG.blade_admin_email
      sign_out @current_user
      @current_user.destroy
      # Added By Harshad Patel
      # Send email to user
      PersonMailer.send_email_of_remove_user_account(email, @current_community).deliver
      # Send email to admin
      PersonMailer.send_email_of_remove_user_account_to_admin(email, admin_email, @current_community).deliver
